...
import시 저장된 경로와 컴포넌트명까지만 기술하기 확장자는 생략한다.
*/
...
function App() {
  /* state를 정의한 후 초기값 'list'를 할당. 이 state를 변경할
  함수는 setMode()로 정의한다.  */
  ...
  /* 3가지 mode에서 공통적으로 props를 통해 state를 변경할 수 
  있는 기능의 화살표 함수를 정의하고 있다. 자식 컴포넌트에서는
  changemode()를 호출해서 해당 기능을 실행한다. */
  if(mode==="view"){
